...
dev: ## Prepare binaries and templates in src dir for faster iteration
	npm run build 
	rsync -av --delete templates/styles/*.css public/styles.css
...
build: ## Build your project and put the output binary in out/bin/
	npm run build 
	npx tailwindcss -o ./templates/styles/output.css 
	GO111MODULE=on $(GOCMD) build  -mod vendor -o out/bin/$(BINARY_NAME) ./cmd/	
	rsync -av --delete js/ out/bin/public/js/
	rsync -av --delete templates/*.html out/bin/templates/
	rsync -av --delete templates/styles out/bin/public/	
...
sqlc: ## Run sqlc generate in sql directory
	sqlc generate && echo "generated coded successfully"
